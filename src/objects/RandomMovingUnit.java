...
public class RandomMovingUnit extends MovableUnit {
	...
	public RandomMovingUnit(Float location, Rectangle rectangle) {
		super(location, rectangle, false);
	}

	public RandomMovingUnit(Float location, Rectangle rectangle, float speed) {
		super(location, rectangle, speed, false);
	}
	...
	public void update() {
		...
		Rectangle thisRectFuture = new Rectangle((int) (location.x + futureX), (int) (location.y + futureY),
		...
		while (it.hasNext()) {
			...
			Rectangle curRect = new Rectangle((int)obj.getBaseLocation().getX(), (int)obj.getBaseLocation().getY(), (int)obj.getRect().getWidth(), (int)obj.getRect().getHeight());
		...
		if (this.direction.equals(Direction.RIGHT)) {
			if (location.x + getRect().width < Toolkit.getDefaultToolkit().getScreenSize().width && !isIntersected) {
				location.x += speed;
			} else {
				this.direction = Direction.LEFT;
			}
		...
		if (this.direction.equals(Direction.BOTTOM)) {
			if (location.y + getRect().height < Toolkit.getDefaultToolkit().getScreenSize().height && !isIntersected) {
				location.y += speed;
			} else {
				this.direction = Direction.TOP;
			}
